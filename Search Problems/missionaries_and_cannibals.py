...
# nested reference is not changed in lists,dict etc by shallowcopy 
# only outerlayer affected shallowcopy a=b[:] or list(a) etc
...
class MissionariesAndCannibals:
    ...
    # Implement mc function
    def mc(self):
        left_bank = self.state[0]
        boat = self.state[1]
        right_bank = self.state[2]
        if boat == 'r':
            # This action requires: Members and cannibals availability on respective bank
            #  members must be greater than equal cannibals check on both banks. There can be zero missionaries as exception
            if (right_bank[0]>=1) and (right_bank[1]>=1) and (left_bank[0]+1 >=left_bank[1]+1) :
            ...
            if (left_bank[0]>=1) and (left_bank[1]>=1) and (right_bank[0]+1 >= right_bank[1]+1):
                right_bank[1] = right_bank[1] + 1
                right_bank[0] = right_bank[0] + 1 
                boat = 'r'
                left_bank[1] = left_bank[1] - 1
